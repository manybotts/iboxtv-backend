...
# Retrieve your credentials and channel info from environment variables
...
# Initialize the Telegram client in bot mode
client = TelegramClient('iboxtv_bot_session', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
...
def fetch_omdb_data(title):
    """
    Calls the OMDb API to get additional show data (poster and plot) using the show title.
    """
    try:
...
def parse_message(message):
    ...
    Returns a dictionary with:
      ...
      - popularity: Defaulted to 0 (adjustable as needed).
    """
    if not message.text:
        return None
    ...
    # Fetch additional details from OMDb API using the show title
